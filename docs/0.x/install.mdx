...
**v0.3.0** is on NuGet. For most apps, packages are enough. Clone the repo when
...
## Scaffold: `dotnet new voluta-agent`

Fastest path to a runnable HITL graph without cloning samples:

```bash
# from NuGet (when Voluta.Templates is published)
...
- `AddVoluta` + **InMemory** checkpointer (swap to File / EF / S3 later)
- One **interrupt** node + automatic `Command.Approve` demo resume
- Optional **MEAI** chat node via `Voluta.Agents.AI` with an **offline stub**
  (`IChatClient`) so the project builds **without an API key**

### Install the template pack from a local pack / path

Contributors and pre-release:
...
# or install the content folder directly (no pack)
dotnet new install ./templates/Voluta.Templates/content
```

Uninstall: `dotnet new uninstall Voluta.Templates` (or the path you passed to
`install`).

Pin Voluta package versions at scaffold time:

```bash
dotnet new voluta-agent -n MyAgent --volutaVersion 0.3.0
```
