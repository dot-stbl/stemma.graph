...
public sealed class GraphStateGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(stateClasses, static (production, symbol) =>
        {
            Execute(production, symbol);
        });
    }
    ...
    private static string GenerateSource(INamedTypeSymbol stateType, ImmutableArray<ChannelMember> channels)
    {
        ...
        sb.AppendLine("using StemmaGraph.Channels;");
        sb.AppendLine("using StemmaGraph.State;");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Partial update type: unset properties emit no write; set (including null) emits a write.");
        sb.AppendLine("    /// </summary>");
        ...
        {
            sb.AppendLine("        /// <summary>");
            sb.Append("        /// Partial update for channel \"").Append(EscapeString(channel.ChannelName)).AppendLine("\".");
            sb.AppendLine("        /// </summary>");
    ...
    private static class Descriptors
    {
        public static readonly DiagnosticDescriptor MustBePartial = new(
            id: "STEMMA001",
            title: "GraphState type must be partial",
            messageFormat: "Type '{0}' is marked with [GraphState] but is not partial",
            category: "StemmaGraph.Generators",
            defaultSeverity: DiagnosticSeverity.Error,
            isEnabledByDefault: true);

        public static readonly DiagnosticDescriptor NoChannels = new(
            id: "STEMMA002",
            title: "GraphState type has no channel properties",
            messageFormat: "Type '{0}' is marked with [GraphState] but has no [Channel] properties",
            category: "StemmaGraph.Generators",
            defaultSeverity: DiagnosticSeverity.Error,
            isEnabledByDefault: true);
    }
