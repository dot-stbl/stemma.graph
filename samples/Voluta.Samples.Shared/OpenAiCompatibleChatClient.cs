...
/// </summary>
/// <remarks>
///     Env: VOLUTA_CHAT_ENDPOINT, VOLUTA_CHAT_API_KEY, VOLUTA_CHAT_MODEL.
/// </remarks>
public sealed class OpenAiCompatibleChatClient(HttpClient http, string model, bool ownsHttp = false) : IChatCompletionClient, IDisposable
{
    ...
    {
        var endpoint = Environment.GetEnvironmentVariable("VOLUTA_CHAT_ENDPOINT");
        var apiKey = Environment.GetEnvironmentVariable("VOLUTA_CHAT_API_KEY");
        var model = Environment.GetEnvironmentVariable("VOLUTA_CHAT_MODEL") ?? "gpt-4o-mini";

        if (string.IsNullOrWhiteSpace(endpoint) || string.IsNullOrWhiteSpace(apiKey))
        {
            client = null;
            error =
                "Set VOLUTA_CHAT_ENDPOINT and VOLUTA_CHAT_API_KEY (optional VOLUTA_CHAT_MODEL), or use --offline.";
            return false;
        }
